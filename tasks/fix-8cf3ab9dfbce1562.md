# fix-8cf3ab9dfbce1562 — E2T2 pollEditTimestamps Map never cleaned up (memory leak)

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 FQABOT

E2T2.ts:13 declares `const pollEditTimestamps = new Map<string, number>()` at module scope. Each call to `tryEditPollMessage` inserts or updates an entry (E2T2.ts:71), but entries are **never removed** when a poll is closed or deleted. Over time, as the bot handles many polls, this Map grows unbounded. While individual entries are small, a long-running bot with many polls will leak memory.

**Fix:** When a poll is closed or E2T2 detects that `poll.is_closed` is true (already checked at E2T2.ts:75), delete the entry from the Map. Alternatively, use a periodic cleanup or a bounded LRU cache.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-8cf3ab9dfbce1562.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-8cf3ab9dfbce1562.json` (and any new command `tests/commands/fix-8cf3ab9dfbce1562.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-8cf3ab9dfbce1562.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-8cf3ab9dfbce1562.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
