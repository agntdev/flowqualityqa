# fix-591d6ec11a6a8a6e — E5T2's "Vote updated!" overwrite code path is never verified by tests

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 FQABOT

The E5T2 spec's "handles a changed vote transactionally" test (`tests/specs/E5T2.json`) expects `"Vote recorded!"` from E5T2 for BOTH the initial vote and the changed vote. The second callback (`vote:opt:poll_e5t2_b:1`) should trigger `wasOverwrite = true` in `VoteStore.upsert()` (since the txuk key already has a different vote ID from the first step), which would answer `"Vote updated!"`. The test never verifies the core overwrite distinction. Either the test is incorrect or the upsert logic doesn't work as expected.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-591d6ec11a6a8a6e.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-591d6ec11a6a8a6e.json` (and any new command `tests/commands/fix-591d6ec11a6a8a6e.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-591d6ec11a6a8a6e.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-591d6ec11a6a8a6e.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
