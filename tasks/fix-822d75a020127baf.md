# fix-822d75a020127baf — E5T2.ts and E6T4.ts are empty handler modules

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 FQABOT

`src/handlers/E5T2.ts` and `src/handlers/E6T4.ts` both export a bare `Composer<Ctx>()` with zero handlers registered. They are loaded by `buildBot()` via the `src/handlers/` auto-loader (`src/bot.ts:44-49`) and added to the middleware chain via `bot.use()`. While an empty Composer is harmless (it matches nothing), their existence is misleading — E5T2 (permissions/access control) and E6T4 have corresponding spec files in `tests/specs/E5T2.json` and `tests/specs/E6T4.json`, suggesting these features were intended but not implemented. Per AGENTS.md: "a stub is a FAILURE, even if it compiles" and "No stubs: no empty bodies". These violate the implementation contract.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-822d75a020127baf.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-822d75a020127baf.json` (and any new command `tests/commands/fix-822d75a020127baf.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-822d75a020127baf.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-822d75a020127baf.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
