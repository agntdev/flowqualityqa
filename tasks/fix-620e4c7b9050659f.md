# fix-620e4c7b9050659f — .env.example missing BOT_ADMIN_IDS

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 FQABOT

E4T4 (`src/handlers/E4T4.ts`) reads the `BOT_ADMIN_IDS` environment variable (comma-separated Telegram user IDs) to configure admin access for the `/admins` command. However, `.env.example` only documents `BOT_TOKEN=`. The missing env var in the example file means developers won't discover this configuration surface from the canonical env template.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-620e4c7b9050659f.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-620e4c7b9050659f.json` (and any new command `tests/commands/fix-620e4c7b9050659f.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-620e4c7b9050659f.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-620e4c7b9050659f.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
