# fix-c5cc9153530ff7fd — E5T1: catch-all callback_query handler responds to ALL callbacks, not just invalid ones

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 FQABOT

`src/handlers/E5T1.ts` registers `composer.on("callback_query", ...)` with no filter. Per the grammY middleware chain in `buildBot()`, this fires for **every** callback query — vote callbacks, poll close/results/export, etc. — each receiving a spurious "Action no longer available" alert alongside their real handler's response. The E5T2 test specs (`tests/specs/E5T2.json`) even hardcode this as an expected extra `answerCallbackQuery` call.

The E5T1 task spec requires handling *invalid or stale* callback queries only. The implementation should either use a negative filter (skip known prefixes like `vote:opt:`, `poll:close:`, etc.) or check whether the callback data corresponds to any valid action before responding.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-c5cc9153530ff7fd.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-c5cc9153530ff7fd.json` (and any new command `tests/commands/fix-c5cc9153530ff7fd.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-c5cc9153530ff7fd.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-c5cc9153530ff7fd.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
