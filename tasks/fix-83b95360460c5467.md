# fix-83b95360460c5467 — Fix: Failing dialog spec: /admins by admin lists admin IDs
...
The Tests-gate replay harness reported spec "/admins by admin lists admin IDs" as failing. 5 spec(s) failed: [show results after poll is closed export non-existent poll /admins by admin lists admin IDs native poll close updates message and removes vote buttons vote on closed poll via poll_answer shows closed message] [3 spec file(s) skipped (empty/unparseable), NOT gated: tests/specs/E1T3.json, tests/specs/E1T4.json, tests/specs/E6T1.json]

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-83b95360460c5467.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-83b95360460c5467.json` (and any new command `tests/commands/fix-83b95360460c5467.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-83b95360460c5467.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-83b95360460c5467.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
