# fix-9cc2c3b3bef0dca9 — E1T2: Unsafe non-null assertion on `ctx.session.poll`

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 FQABOT

At `src/handlers/E1T2.ts:16`: `ctx.session.poll!.question = text;` — the `!` non-null assertion bypasses null-safety without a defensive guard. If a future handler (or session corruption) sets `step` to `"awaiting_poll_question"` without initializing `poll`, this will throw a `TypeError` and crash the handler chain. A preceding `if (!ctx.session.poll) return;` guard should be added before the assignment.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-9cc2c3b3bef0dca9.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-9cc2c3b3bef0dca9.json` (and any new command `tests/commands/fix-9cc2c3b3bef0dca9.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-9cc2c3b3bef0dca9.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-9cc2c3b3bef0dca9.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
