# fix-c97062f833048d18 — E5T3 dialog tests do not verify actual closed-poll behavior

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 FQABOT

Both specs in `tests/specs/E5T3.json` reference poll IDs (`poll_closed_1`, `poll_closed_2`) that don't exist in any store. The handler returns early when `pollStore.getById()` returns `null`:
- Test 1 (poll close update): handler exits at `if (!poll) return` on line 23 — never reaches `editMessageText`
- Test 2 (poll_answer on closed poll): handler exits at `if (!poll?.is_closed) return` on line 71 — never sends "Poll is closed" message

Both tests expect `[]` (no API calls) and pass trivially. The spec file needs store pre-population (e.g., via a `setup` step or handler-side fixtures) to exercise the actual edit/closing logic.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-c97062f833048d18.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-c97062f833048d18.json` (and any new command `tests/commands/fix-c97062f833048d18.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-c97062f833048d18.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-c97062f833048d18.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
