# fix-424eb63b33e812a0 — E2T1.ts silently records votes for non-existent polls

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 FQABOT

In `src/handlers/E2T1.ts:24-28`, the vote handler checks `if (poll && poll.is_closed)` to block votes on closed polls. However, when `poll` is `null` (poll doesn't exist in store), the condition is false and execution continues, recording a vote for a poll that doesn't exist. This creates orphaned vote records in storage. The spec (docs/spec.md line 81) requires: "Invalid callbackQuery (stale message, removed poll, malformed data): answerCallbackQuery with a short message: 'Action no longer available'". For a non-existent poll, the handler should answer with an alert and return early.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-424eb63b33e812a0.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-424eb63b33e812a0.json` (and any new command `tests/commands/fix-424eb63b33e812a0.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-424eb63b33e812a0.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-424eb63b33e812a0.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
